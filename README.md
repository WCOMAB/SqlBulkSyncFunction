...
| `SyncJobsConfig__Jobs__[key]__UseSnapshotIsolationSeed`                    | Optional. When `true`, seed bulk copy reads source under Snapshot isolation (no `NOLOCK`). Requires `ALLOW_SNAPSHOT_ISOLATION` on source DB.       | `true` or `false`                                                            |
| `SyncJobsConfig__Jobs__[key]__UseApplicationIntentReadOnlySeed`            | Optional. When `true`, seed bulk copy uses a separate source connection with `ApplicationIntent=ReadOnly` (ReadScale). Schema/CT stay on primary.  | `true` or `false`                                                            |
| `SyncJobsConfig__Jobs__[key]__FullSync__IntervalMilliseconds`              | Optional opt-in. When any `FullSync__*` key is set, the job uses interval full sync (no change tracking). Minimum ms between syncs; must be `> 0`. | `3600000`                                                                    |
| `SyncJobsConfig__Jobs__[key]__FullSync__UseSnapshotIsolation`              | Optional. Full-sync source reads under Snapshot isolation (no `NOLOCK`). Requires `ALLOW_SNAPSHOT_ISOLATION` on source DB.                         | `true` or `false`                                                            |
| `SyncJobsConfig__Jobs__[key]__FullSync__UseApplicationIntentReadOnly`      | Optional. Full-sync source bulk reads use `ApplicationIntent=ReadOnly` (read scale-out). Version/clock stays on primary.                           | `true` or `false`                                                            |
...
>
> **UseSnapshotIsolationSeed**: Omit or set to `false` to keep the default seed read (`WITH(NOLOCK)`, no transaction). Set to `true` for a consistent seed snapshot of high-churn tables; the source database must have `ALTER DATABASE … SET ALLOW_SNAPSHOT_ISOLATION ON`.
>
> **UseApplicationIntentReadOnlySeed**: Omit or set to `false` to seed from the primary source connection (default). Set to `true` to offload seed bulk reads to a readable secondary (`ApplicationIntent=ReadOnly`); requires ReadScale (or equivalent) on the source. Change-tracking metadata is still read from the primary. Be aware of replica lag: the persisted CT version comes from the primary, so a lagged secondary seed can leave the target missing rows that incremental sync will not replay.
>
> **FullSync**: Omit the entire `FullSync` section to keep change-tracking sync (default, production-safe). When any `FullSync__*` key is present, the job opts into interval full sync for **all** tables in that job: no `CHANGE_TRACKING` / `CHANGETABLE`. `sync.TableVersion.CurrentVersion` stores Unix epoch milliseconds from the source clock. Scheduled runs sync only when `(nowMs - lastMs) >= IntervalMilliseconds` (or the table was never synced). When due, the target is reconciled without clearing: orphan delete, move new rows from staging into target, then update only changed matches. Explicit seed (`POST queue/{area}/{id}/true`) always clears and bulk-loads, ignoring the interval. `FullSync__UseSnapshotIsolation` / `FullSync__UseApplicationIntentReadOnly` apply only to this mode (not the `*Seed` flags). Schema tracking / export HTTP endpoints remain change-tracking based and are not intended for FullSync jobs. Do not flip a job between CT and FullSync without clearing `sync.TableVersion` for its tables.
>
